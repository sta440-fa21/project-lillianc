...
```{r fig.height=3}
jailtimeserved_barplot <- prison %>%
  ggplot(aes(x = jail_time_served, fill = jail_time_served)) +
...
jailtime_histogram <- prison %>%
  filter(!is.na(jail_time), jail_time < 11 * 365, jail_time_served == "Yes") %>%
  ggplot(aes(x = jail_time)) +
  geom_histogram(fill = cols[[2]], color = "black", binwidth = 150) +
...
get_nas("age_at_arrest")
get_nas("arrest_year_range")
get_nas("admit_year")
get_nas("sex")
get_nas("race")
get_nas("citizen")
get_nas("military")
get_nas("education")
get_nas("homeless_12mo_prior")
get_nas("held_by")
get_nas("sentenced")
get_nas("controlling_offense")
get_nas("controlling_offense_type")
get_nas("arrested_during_status")
get_nas("firearm_at_offense")
get_nas("alc_at_offense")
get_nas("drug_at_offense")
get_nas("state")
```

**Figure 3: Distribution of Predictors**

```{r}
prison %>%
  filter(is.na(jail_time_served)) %>%
  count(jail_time_served, sentenced)
```
